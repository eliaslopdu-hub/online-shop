/* =============================================================================
   Udara Resonance — 3D tuning-fork hero.
   Vanilla JS module. Three.js + GSAP/ScrollTrigger (CDN) + Web Audio API.
...
function initScene() {
  ...
  if (window.gsap && window.ScrollTrigger) {
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        // Scroll "strikes" the fork → stronger ripple as you move.
        triggerPulse(Math.abs(self.getVelocity()) * 0.00002);
      },
      onToggle: (self) => { heroVisible = self.isActive || self.progress < 1; },
    });

    // Seamless 3D→2D handoff. The whole hero must DISSOLVE to plain cream before
    // the pin releases — otherwise the title/strike/grain "pop" away and the seam
    // is felt. Everything fades on opacity only (no transforms → no centring
    // breakage), finishing just before the hero ends, so the content (also cream)
    // emerges with no visible boundary.
    const heroCopy   = document.querySelector('.udara-hero__copy');
    const heroGrain  = document.querySelector('.udara-grain');
    const heroVig    = document.querySelector('.udara-vignette');
    const heroHint   = document.getElementById('udara-scrollhint');

    // Copy + strike + scroll hint fade early, so the long middle of the scroll is
    // just the fork turning slowly and the waves breathing — nothing else.
    gsap.to([heroCopy, strikeBtn, heroHint], {
      opacity: 0,
      ease: 'power1.in',
      scrollTrigger: { trigger: heroEl, start: 'top top', end: '24% top', scrub: true },
    });
    // NOTE: the canvas (fork + waves) is NO LONGER faded here. The fork dissolves
    // and the wave field lingers over the start of the text, both driven by raw
    // scrollY in the render loop (see "linger" below) so they can outlive the hero.
    gsap.to([heroGrain, heroVig], {
      opacity: 0,
      ease: 'none',
      scrollTrigger: { trigger: heroEl, start: '70% top', end: '90% top', scrub: true },
    });
  }
  ...
  // Pause the loop only when BOTH the hero AND the first text section are off
  // screen — so the wave field keeps rendering while it lingers over the text,
  // then pauses for battery/perf once we're past the handoff.
  const visibleTargets = new Set();
  const io = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
      e.isIntersecting ? visibleTargets.add(e.target) : visibleTargets.delete(e.target);
    });
    visibleTargets.size > 0 ? start() : stop();
  }, { threshold: 0.01 });
  io.observe(heroEl);
  const storyEl = document.querySelector('.udara-sec--story');
